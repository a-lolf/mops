...
def CP_Jun_SW_ChassisFPC(input_json):  # NOTE: Function name and file name have to be EXACTLY the same!
    """
    Description:
    ...
    try:
        ...
        for line in commandOutput.splitlines():
            match = re.search(r"\d+\s+\w+\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+(\d+)", line)
...
#   0  Online            26      7          0        7      6      6    16384      28         49
#   1  Empty           
#   2  Empty           
#   3  Empty           
#   4  Empty           
...
# a = CP_Jun_SW_ChassisFPC(input_json)
# print(a)
