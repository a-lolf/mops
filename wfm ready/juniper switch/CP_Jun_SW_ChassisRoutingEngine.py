...
def CP_Jun_SW_ChassisRoutingEngine(input_json):  # NOTE: Function name and file name have to be EXACTLY the same!
    ...
    try:
        ...
        matches = re.findall(regex, commandOutput)
        ...
        #     headers = [
        #         'Region', 'CustomNodeName', 'NodeName', 'Timestamp',]

            data_to_write = [commandOutput]

            # common_fields = [region, customNodeName, nodeName, timestamp]

            # for match in matches:
            #     current_state, master = match
            #     data_to_write.append(common_fields + [item, current_state, master])
        ...
        if str(details).lower()[0] == 'y':
            if os.name == "posix":
                ...
                try:
                    ...
                    file_path = f"show_system_alarm_{sanitized_timestamp}.csv"
